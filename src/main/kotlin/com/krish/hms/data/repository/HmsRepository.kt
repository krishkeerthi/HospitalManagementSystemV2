...
class HmsRepository(val uiHandler: UIHandler) {
    ...
    fun assignDoctor(ssn: Int, issue: String, time: LocalTime){
        ...
        if(departmentDoctors.isEmpty())
            return uiHandler.writeData("No doctors available", LogLevel.WARNING)
        else{
            ...
            if(assignedDoctorId != "")
                ...
                uiHandler.writeData("No doctors available", LogLevel.WARNING)
        }
    }
    ...
    fun giveMedicines(doctorId: String){
        ...
        if(consultationId == null)
            return uiHandler.writeData("No consultations to handle", LogLevel.WARNING)
        else{
